...
# PARAMETERS
my $resolution = 0.3; # 1.1; # 1.2;
my $min_logfc = 0; # 1;
my $total_no_of_genes_with_this_go_term = 0;

# REQUIREMENTS
my $in_dir = "/project/GorielyLab2021/sbush/human_adult_SSCs/results/states0to4.go_enrichment_per_cluster.resolution_$resolution.min_log_fc_$min_logfc"; # from 14b.run_topgo_on_cluster_markers_for_SSC_atlas.pl
if (!(-d($in_dir))) { print "ERROR: cannot find $in_dir\n"; exit 1; }

# OUTPUT
my $out_file = "/project/GorielyLab2021/sbush/human_adult_SSCs/results/states0to4.go_enrichment_per_cluster.resolution_$resolution.min_log_fc_$min_logfc.txt";
