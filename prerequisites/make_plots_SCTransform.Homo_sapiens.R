...
# the t2g file is at /project/GorielyLab2021/sbush/ssc_atlas/indexes/Homo_sapiens/t2g2.txt
...
tr2g <- read_tsv("/project/GorielyLab2021/sbush/ssc_atlas/indexes/Homo_sapiens/t2g2.txt", col_names = c("transcript", "gene", "gene_name")) # HARD-CODED
tr2g <- distinct(tr2g[, c("gene", "gene_name")])
rownames(res_mat) <- tr2g$gene_name[match(rownames(res_mat), tr2g$gene)]
(testis <- CreateSeuratObject(counts = res_mat, project = "testis", min.cells = 3, min.features = 500))
...
### FILTER CELLS ON THE BASIS OF, E.G., nFeature_RNA, nCount_RNA, AND percent.mt

num_cells_nFeature_RNA_le_1000  <- nrow(subset(testis@meta.data,nFeature_RNA <= 1000))
num_cells_nFeature_RNA_ge_10000 <- nrow(subset(testis@meta.data,nFeature_RNA >= 10000))
num_cells_nCount_RNA_le_2000    <- nrow(subset(testis@meta.data,nCount_RNA <= 2000))
num_cells_nCount_RNA_ge_50000   <- nrow(subset(testis@meta.data,nCount_RNA >= 50000))
num_cells_percentMT_ge_5        <- nrow(subset(testis@meta.data,percent.mt >= 5))

testis <- subset(testis, subset = nFeature_RNA > 1000 & nFeature_RNA < 10000 & nCount_RNA > 2000 & nCount_RNA < 50000 & percent.mt < 5)
num_cells_passing_all_QC_thresholds <- nrow(testis@meta.data)
...
total_df<-data.frame(NUM_CELLS_KB_UNFILTERED=num_cells_in_kb_unfiltered,NUM_CELLS_KB_FILTERED=num_cells_in_kb_filtered,NUM_CELLS_WITH_LE_1000_GENES=num_cells_nFeature_RNA_le_1000,NUM_CELLS_WITH_GE_10k_GENES=num_cells_nFeature_RNA_ge_10000,NUM_CELLS_WITH_LE_2k_UMIS=num_cells_nCount_RNA_le_2000,NUM_CELLS_WITH_GE_50k_UMIS=num_cells_nCount_RNA_ge_50000,NUM_CELLS_WITH_PCT_MT_GE_5=num_cells_percentMT_ge_5,NUM_CELLS_PASSING_ALL_QC_FILTERS=total_cells,NUM_GENES_KB_FILTERED=num_genes_in_kb_filtered,NUM_GENES_PASSING_ALL_QC_FILTERS_AND_IN_GE_3_CELLS=total_genes)
...
### OUT11: convert the Seurat object to an h5ad object for visualisation with cellxgene

DefaultAssay(testis) <- 'RNA'
testis <- NormalizeData(testis)
testis <- FindVariableFeatures(testis, selection.method = "vst", nfeatures = 5000)
all.genes <- rownames(testis)
testis <- ScaleData(testis, features = all.genes)
testis@meta.data$barcode<-'NULL'
testis@meta.data$old.ident<-'NULL'

sceasy::convertFormat(testis, from="seurat", to="anndata", outFile='R_SCT/testis.h5ad')