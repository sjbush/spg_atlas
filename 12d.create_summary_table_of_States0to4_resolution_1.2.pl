...
my $total_sscs_in_atlas  = 2234; # i.e. the number of cells in clusters 0, 2, 5 and 6. Count can be obtained from states0to4.metadata.txt, produced by 8.subset_whole_testes_atlas_to_obtain_States0to4.R
my $total_non_sscs_in_atlas = $total_cells_in_atlas-$total_sscs_in_atlas;
my @cluster_order = ("Cluster 2","Cluster 0","Cluster 5","Cluster 6","Cluster 1","Cluster 7","Cluster 3","Cluster 4"); # this relates only to the order in which the DE results are presented
...
my $cluster4_name = 'ZYGOTENE';
my $cluster5_name = 'STATE 0/0B';
my $cluster6_name = 'STATE 0B/1';
my $cluster7_name = 'DIFF SPG';
my %cluster_names = (
...
'Cluster 3' => 'leptotene',
'Cluster 4' => 'zygotene',
'Cluster 5' => 'state 0/0B',
'Cluster 6' => 'state 0B/1',
'Cluster 7' => 'diff SPG'
);
...
my $proporp8   	   = "/project/GorielyLab2021/sbush/human_adult_SSCs/results/states0to4.proporp_of_cells_expressing_each_gene_in_Cluster7_resolution_$res.txt";  # from 11.count_num_of_cells_per_cluster_in_which_each_gene_is_expressed.R
...
my $num8  	   	   = "/project/GorielyLab2021/sbush/human_adult_SSCs/results/states0to4.number_of_cells_expressing_each_gene_in_Cluster7_resolution_$res.txt";   # from 11.count_num_of_cells_per_cluster_in_which_each_gene_is_expressed.R
...
my $expr8  	   	   = "/project/GorielyLab2021/sbush/human_adult_SSCs/results/states0to4.avg_expr_of_cells_expressing_each_gene_in_Cluster7_resolution_$res.txt"; # from 11.count_num_of_cells_per_cluster_in_which_each_gene_is_expressed.R
...
my $fatal 		   = 0;
my @in_files = ($pheno_file,$id_to_reactome,$reactome_descs,$orthologues,$markers,$genelist1,$genelist2,$genelist3,$genelist4,$genelist5,$genelist6,$t2g,$proporp1,$proporp2,$proporp3,$proporp4,$proporp5,$proporp6,$proporp7,$proporp8,$num1,$num2,$num3,$num4,$num5,$num6,$num7,$num8,$expr1,$expr2,$expr3,$expr4,$expr5,$expr6,$expr7,$expr8,$twist_no_cov);
...
print OUT "Total number of undifferentiated SSCs (i.e. CLUSTER 0 + CLUSTER 2 + CLUSTER 5 + CLUSTER 6) in which this gene is detected\t% of undifferentiated SSCs (i.e. CLUSTER 0 + CLUSTER 2 + CLUSTER 5 + CLUSTER 6) in which this gene is detected\t";
print OUT "Total number of non-SSC cells in which this gene is detected (i.e. all except CLUSTERS 0, 2, 5 and 6)\t% of non-SSC cells in which this gene is detected (i.e. all except CLUSTERS 0, 2, 5 and 6)\t";
...
foreach my $gene_name (@sorted_gene_names)
	{ my $gene_id = '.';
	  ...
	  my $tot_cells_in_which_gene_detected    = $ct{$gene_name}{'Cluster 0'}+$ct{$gene_name}{'Cluster 1'}+$ct{$gene_name}{'Cluster 2'}+$ct{$gene_name}{'Cluster 3'}+$ct{$gene_name}{'Cluster 4'}+$ct{$gene_name}{'Cluster 5'}+$ct{$gene_name}{'Cluster 6'}+$ct{$gene_name}{'Cluster 7'};
	  my $pct_cells_in_which_gene_detected    = sprintf("%.3f",(($tot_cells_in_which_gene_detected/$total_cells_in_atlas)*100));
	  my $tot_sscs_in_which_gene_detected     = $ct{$gene_name}{'Cluster 0'}+$ct{$gene_name}{'Cluster 2'}+$ct{$gene_name}{'Cluster 5'}+$ct{$gene_name}{'Cluster 6'};
	  my $pct_sscs_in_which_gene_detected     = sprintf("%.3f",(($tot_sscs_in_which_gene_detected/$total_sscs_in_atlas)*100));
	  my $tot_non_sscs_in_which_gene_detected = $ct{$gene_name}{'Cluster 1'}+$ct{$gene_name}{'Cluster 3'}+$ct{$gene_name}{'Cluster 4'}+$ct{$gene_name}{'Cluster 7'};
	  ...
	  my $orthologue_line = ".\t.\t.\t.\t.\t.";
	  my $de_in_cluster0 = 'no'; my $de_in_cluster1 = 'no'; my $de_in_cluster2 = 'no'; my $de_in_cluster3 = 'no'; my $de_in_cluster4 = 'no'; my $de_in_cluster5 = 'no'; my $de_in_cluster6 = 'no'; my $de_in_cluster7 = 'no';
	  ...
	  if ($number_of_clusters_in_which_this_gene_is_de == 0)
		{ print OUT "$gene_name\t$gene_id\t$gene_desc\t$gene_type\t$loc\t$orthologue_line\t$phenotype\t$source\t$reactome_ids\t$reactome_descs\t$is_this_gene_on_the_oncogenic_mapk_pathway\t";
		  print OUT "$ct{$gene_name}{'Cluster 2'}\t$ct{$gene_name}{'Cluster 0'}\t$ct{$gene_name}{'Cluster 5'}\t$ct{$gene_name}{'Cluster 6'}\t$ct{$gene_name}{'Cluster 1'}\t$ct{$gene_name}{'Cluster 7'}\t$ct{$gene_name}{'Cluster 3'}\t$ct{$gene_name}{'Cluster 4'}\t";
		  ...
		  print OUT "$pct{$gene_name}{'Cluster 2'}\t$pct{$gene_name}{'Cluster 0'}\t$pct{$gene_name}{'Cluster 5'}\t$pct{$gene_name}{'Cluster 6'}\t$pct{$gene_name}{'Cluster 1'}\t$pct{$gene_name}{'Cluster 7'}\t$pct{$gene_name}{'Cluster 3'}\t$pct{$gene_name}{'Cluster 4'}\t";
		  print OUT "$avg{$gene_name}{'Cluster 2'}\t$avg{$gene_name}{'Cluster 0'}\t$avg{$gene_name}{'Cluster 5'}\t$avg{$gene_name}{'Cluster 6'}\t$avg{$gene_name}{'Cluster 1'}\t$avg{$gene_name}{'Cluster 7'}\t$avg{$gene_name}{'Cluster 3'}\t$avg{$gene_name}{'Cluster 4'}\t";
		  print OUT "$number_of_clusters_in_which_this_gene_is_de\t.\t.\t.\t.\t.\t.\t.\t.\t.\t";
		  print OUT "$de_in_cluster2\t$de_in_cluster0\t$de_in_cluster5\t$de_in_cluster6\t$de_in_cluster1\t$de_in_cluster7\t$de_in_cluster3\t$de_in_cluster4\t";
